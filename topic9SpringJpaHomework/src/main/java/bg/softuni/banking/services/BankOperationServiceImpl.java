...
public class BankOperationServiceImpl implements BankOperationService{
	...
	public List<String> validateOperation(Operation operation) {
		...
		if (Constants.ROLE_USER.equals(operation.getUser().getRole()) && !operation.getUser().getUsername().equals(operation.getAccount().getUser().getUsername())) errors.add(ERROR_OPERATION_NOT_ALOWED);
	...
	public boolean calculateAmount(Operation operation) {
		...
		if (deposit) {
			operation.setOperationType(operationDao.getOperationTypeByName("deposit"));
			...
			operation.setOperationType(operationDao.getOperationTypeByName("withdraw"));
